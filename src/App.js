...
const NeoBrutalismCrudoCotto = () => {
  // Dati delle tabelle di conversione completi
  ...
  const categoryKeys = Object.keys(conversionData || {});
  const firstCategory = categoryKeys.length > 0 ? categoryKeys[0] : '';
  
  // Verifica sicura del primo alimento
  let firstFoodItem = { alimento: '', fattore: 1, info: '', tip: '' };
  if (firstCategory && 
      conversionData && 
      conversionData[firstCategory] && 
      Array.isArray(conversionData[firstCategory]) && 
      conversionData[firstCategory].length > 0) {
    firstFoodItem = conversionData[firstCategory][0];
  }
  
  // Stati dell'applicazione con inizializzazioni sicure
  ...
  useEffect(() => {
    const preventZoom = () => {
      try {
        // Impostazioni sicure per prevenire lo zoom
        if (document && document.documentElement) {
          document.documentElement.style.touchAction = 'manipulation';
        }
        
        // Verifica specifica per dispositivi iOS
        ...
        // Impostazione sicura del viewport
        const viewportMeta = document && document.querySelector('meta[name="viewport"]');
        if (viewportMeta) {
    ...
    // Chiamiamo preventZoom all'inizio
    preventZoom();
    
    // Aggiungiamo listener per il focus sull'input in modo sicuro
      ...
      try {
        ...
        return () => {
          try {
            ...
            console.error('Errore durante la rimozione degli event listener:', error);
          }
        };
      } catch (error) {
        console.error('Errore durante l\'aggiunta degli event listener:', error);
      }
    }
  }, []);
  ...
  useEffect(() => {
    // Verifiche di sicurezza multiple
    if (!categoria || !alimento || !conversionData) return;
    
    const categoriaData = conversionData[categoria];
    if (!categoriaData || !Array.isArray(categoriaData)) return;
    
    try {
      // Trova l'alimento in modo sicuro
      const alimentoScelto = categoriaData.find(item => 
        item && typeof item === 'object' && item.alimento === alimento
      );
      
      if (alimentoScelto) {
        setFattore(Number(alimentoScelto.fattore) || 1);
  ...
  }, [alimento, categoria, conversionData]);
  
  // Effetto di scrolling al risultato
  useEffect(() => {
    if (risultato && risultatoRef && risultatoRef.current) {
      try {
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (infoRef && infoRef.current && !infoRef.current.contains(event.target) && 
          infoButtonRef && infoButtonRef.current && !infoButtonRef.current.contains(event.target)) {
  ...
  const handleCategoriaChange = (e) => {
    try {
      ...
      // Verifica la presenza della categoria e degli alimenti in modo sicuro
      if (conversionData && 
          nuovaCategoria in conversionData && 
